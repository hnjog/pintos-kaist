...
struct thread
{
	...
	/* 4BSD */
	int nice;
	int recent_cpu; /* 최근에 얼마나 많은 CPU를 사용했는가를 표현 */
...
int64_t get_next_tick_to_awake(void); /* return get_next_tick_to_awake */
int64_t global_next_ticks_to_awake;	  /* minumum value of the wakeup ticks */

void test_max_priority(void);				 /* 현재 수행중인 스레드와 가장높은 우선순위의 스레드의 우선순위를 비교하여 스케줄링*/
