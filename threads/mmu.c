...
/*
 ...
 palloc으로 할당받은 page를 (0으로 초기화 된 것)
 vtop로 바꾸어
 pml4e에 넣어준다 (아마 t->pml4 인 녀석)
*/
uint64_t *
...
해당 물리적 주소에 해당하는 커널 가상 주소를 반환하거나 UADDR이 매핑되지 않은 경우 Null 포인터를 반환합니다.
*/
void *
 ...
 * failed. */
/*
사용자 가상 페이지 UPAGE에서 커널 가상 주소 KPAGE로 식별된 물리적 프레임에 페이지 맵 레벨 4 PML4의 매핑을 추가합니다.
UPAGE는 이미 매핑되어 있지 않아야 합니다. KPAGE는 palloc_get_page()가 있는 사용자 풀에서 가져온 페이지여야 합니다.
WRITABLE이 true이면 새 페이지가 읽기/쓰기됩니다;
그렇지 않으면 읽기 전용입니다.
